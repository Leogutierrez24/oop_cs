...
namespace ejercicio06
{
    public class Hotel
    {
        ...
        private List<Tuple<DateTime, Habitacion>> _apartadoHabitaciones = new List<Tuple<DateTime, Habitacion>>();
            ...
            float total)
        {
            ...
            if (this.ComprobarDisponibilidad(nuevaReserva) != -1)
            {
                ...
                this._recaudacion += comprobanteReserva.Importe;
            } else
            {
                nuevaReserva = null;
            }
        ...
        public int RegistrarOcupacion(Reserva reserva)
        {
            ...
            if (reserva.CheckinEstado != true && reserva.CheckoutEstado != true)
            {
                ...
                this._apartadoHabitaciones.Add(new Tuple<DateTime, Habitacion>(DateTime.Now, reserva.Habitacion));
                resultado = 0;
            } else
            {
                resultado = -1;
        ...
        public int RegistrarDesocupacion(Reserva reserva)
        {
            ...
            if (reserva.CheckinEstado != false && reserva.CheckoutEstado != true)
            {
                ...
                this._recaudacion += nuevoComprobante.Importe;
        ...
        private int CalcularRepeticionHabitacion(List<Reserva> lista, Habitacion habitacion)
        {
            int repeticiones = 0;

            lista.ForEach(reserva =>
            {
                if (reserva.Habitacion.Numero == habitacion.Numero)
                {
                    repeticiones++;
                }
            });
        ...
        private int CalcularRepeticionHuesped(List<Reserva> lista, Huesped huesped)
        {
            int repeticiones = 0;

            lista.ForEach(reserva =>
            {
                if (reserva.Ocupantes.Exists(ocupante => ocupante.Documento == huesped.Documento))
                {
                    repeticiones++;
                }
            });
        ...
        private int CalcularRepeticionApartado(List<Tuple<DateTime, Habitacion>> lista, Habitacion habitacion)
        {
            int repeticiones = 0;

            lista.ForEach(apartado =>
            {
                if (apartado.Item2.Numero == habitacion.Numero)
                {
                    repeticiones++;
                }
            });
        ...
        public Habitacion ObtenerHabitacionMasSolicitadaPorFecha(DateTime inicial, DateTime final)
        {
            Habitacion masElegida = null;
            int vecesElegida = 0;
            List<Reserva> allReservas = new List<Reserva>();
            List<Reserva> reservasEnRango = new List<Reserva>();

            this._reservas.ForEach(reserva => allReservas.Add(reserva));
            this._reservasConcretadas.ForEach(reserva => allReservas.Add(reserva));

            reservasEnRango = allReservas.FindAll(reserva => reserva.FechaEntrada >= inicial && reserva.FechaEntrada <= final);

            this._habitaciones.ForEach(habitacion =>
            {
                int repeticiones = this.CalcularRepeticionHabitacion(reservasEnRango, habitacion);
                if (repeticiones > vecesElegida)
                {
                    masElegida = habitacion;
                }
            });
        ...
        public Habitacion ObtenerHabitacionMasOcupadaPorFecha(DateTime inicial, DateTime final)
        {
            Habitacion masOcupada = null;
            int vecesElegida = 0;
            List<Tuple<DateTime, Habitacion>> apartadosEnRango = new List<Tuple<DateTime, Habitacion>>();

            apartadosEnRango = this._apartadoHabitaciones.FindAll(apartado => apartado.Item1 >= inicial && apartado.Item1 <= final);

            this._habitaciones.ForEach(habitacion =>
            {
                int repeticiones = this.CalcularRepeticionApartado(apartadosEnRango, habitacion);
                if (repeticiones > vecesElegida)
                {
                    masOcupada = habitacion;
                }
            });
        ...
        public Huesped ObtenerHuespedMasHospedaje()
        {
            Huesped masHospedaje = null;
            int vecesHospedado = 0;

            if (this._reservasConcretadas.Count != 0)
            {
                this._huespedes.ForEach(huesped =>
                {
                    int repeticiones = this.CalcularRepeticionHuesped(this._reservasConcretadas, huesped);
                    if (repeticiones > vecesHospedado)
                    {
                        masHospedaje = huesped;
                    }
                });
            }
        ...
        public float CalcularRecaudacionPorFecha(DateTime inicio, DateTime final)
        {
            float recaudacion = 0;
            List<Comprobante> comprobantesEnRango = new List<Comprobante>();

            if (this._comprobantes.Count != 0)
            {
                comprobantesEnRango = this._comprobantes.FindAll(comprobante => comprobante.Fecha >= inicio && comprobante.Fecha <= final);
                comprobantesEnRango.ForEach(comprobante =>
                {
                    recaudacion += comprobante.Importe;
                });
            }
